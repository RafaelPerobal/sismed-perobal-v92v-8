...
const PrescriptionDateSelector = ({
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={toggleShowOptions} className="text-health-600 font-normal text-xl mx-0">
          {showOptions ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
        </Button>
      </div>
      
      {showOptions && <Card className="p-4">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {dates.map((dateConfig, index) => <div key={index} className="flex items-center space-x-2">
                  <Checkbox id={`date-enabled-${index}`} checked={dateConfig.enabled} onCheckedChange={() => handleToggleDate(index)} />
                  <div className="grid grid-cols-3 gap-2 flex-grow">
                    <div className="col-span-2">
                      <Input type="date" value={dateConfig.date} onChange={e => handleDateChange(index, e.target.value)} disabled={!dateConfig.enabled} />
                    </div>
                    <div>
                      <Button type="button" variant="ghost" size="icon" className="text-red-500 hover:text-red-700" onClick={() => removeDate(index)}>
            ...
            {dates.length < 6 && <Button type="button" variant="outline" size="sm" onClick={addDate} className="w-full">
                <CalendarIcon className="h-4 w-4 mr-2" />
                Adicionar data
              </Button>}
            
            <div className="text-sm text-muted-foreground">
              {dates.filter(d => d.enabled).length > 0 ? `Serão geradas ${dates.filter(d => d.enabled).length} receitas separadas` : 'Nenhuma receita selecionada'}
            </div>
          </div>
        </Card>}
    </div>;
